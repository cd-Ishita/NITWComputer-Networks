...
int main(){
    ...
    while(1){
        ...
        struct msg msq;
        msq.type=1; //the type of receiver and sender should match for success
        
        if(verify == 0){
            strcpy(msq.buf,"Do not Verify! -from A");
        }
        else{
            strcpy(msq.buf, "Verify! -from A");
        }
