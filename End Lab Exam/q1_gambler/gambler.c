...
void packet_handler(u_char* args,const struct pcap_pkthdr* header,const u_char* packet){
	...
	if(ntohs(eth_hdr->ether_type)==ETHERTYPE_IP){
		...
		strcpy(IP[my_index],inet_ntop(AF_INET,&(iph->saddr),ad,INET_ADDRSTRLEN));
        printf("Sender IP: %s\n",IP[my_index]);

        strcpy(MAC[my_index], ether_ntoa((const struct ether_addr*)eth_hdr->ether_shost));
        ...
        */
        ...
        for(int i=0;i<4;i++){
            char temp[2048];
            sprintf(temp, "%d", (int)arph->__ar_sip[i]);
            strcat(result, arph->__ar_sip[i]);
            strcat(result, ".");
        }

        /*printf("%s", result);
        
        strcpy(IP[my_index],result);
        printf("Sender IP: %s\n",IP[my_index]);

        MAC[my_index] = ether_ntoa((const struct ether_addr*)eth_hdr->ether_shost);
        printf("Sender MAC Address: %s\n",MAC[my_index]);
        my_index++;
        printf("--------------------------------------------\n\n");
        /*
